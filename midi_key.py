...
def handle_key_press(key, notes_0, notes_1, notes_2, notes_3):
    ...
    if key == pygame.K_z:
        threading.Thread(target=play_note_with_instrument, args=(note_0, 0.4, 127, 'Piano')).start()
    elif key == pygame.K_x:
        threading.Thread(target=play_note_with_instrument, args=(note_1, 0.4, 127, 'Piano')).start()
    elif key == pygame.K_c:
        threading.Thread(target=play_note_with_instrument, args=(note_2, 0.4, 127, 'Piano')).start()
    elif key == pygame.K_v:
        threading.Thread(target=play_note_with_instrument, args=(note_3, 0.4, 127, 'Piano')).start()
